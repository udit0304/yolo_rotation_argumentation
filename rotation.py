...
for image_name0 in image_names:
  ...
  for angle in range(0,360,angle_interval):
    ...
    if angle == 0.:
      ...
      nW = int((height_image * sin) + (width_image * cos))
      nH = int((height_image * cos) + (width_image * sin))
      ...
      image = cv2.warpAffine(image0,matrix,(nW,nH),borderMode=cv2.BORDER_REFLECT_101)
    ...
    for coord in coords:
      ...
      x_left   = constraint(x_left  ,0,nW)
      x_right  = constraint(x_right ,0,nW)
      y_top    = constraint(y_top   ,0,nH)
      y_bottom = constraint(y_bottom,0,nH)
      ...
      if area > area0*ratio:
        ...
        if not show_image:
          with open(label_name,"a") as f1:
            label = coord2label([category, x_left, y_top, x_right, y_bottom],nH,nW)
